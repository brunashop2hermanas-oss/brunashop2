...
function addModals() {
  ...
  // Add state
  if (!content.includes('const [tallaEditando, setTallaEditando] = useState')) {
  ...
  // Replace Edit action
  const editActionSearch = 'const nuevoNombre = window.prompt("Editar nombre de la talla:", talla);';
  const editActionReplace = 'setTallaEditando({old: talla, new: talla}); return;';
  
  if (content.includes(editActionSearch)) {
      // Find the whole block
      const blockRegex = /const nuevoNombre = window\.prompt\([\s\S]*?setStockPorTalla\(newStockPorTalla\);\s*\}\s*\}/g;
      content = content.replace(blockRegex, editActionReplace);
  }

  // Replace Delete action
  const deleteActionSearch = 'if (confirm(`¿Estás seguro de eliminar la talla personalizada "${talla}"?`)) {';
  const deleteActionReplace = 'setTallaABorrar(talla); return;\n                                          if (false) {';
  
  if (content.includes(deleteActionSearch)) {
      content = content.replace(deleteActionSearch, deleteActionReplace);
  }
  ...
  if (!content.includes('TALLA EDIT MODAL')) {
...
addModals();
console.log('done modal patching');
